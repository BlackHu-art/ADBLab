...
| High | Controller 用共享字段管理同类操作，重叠批次/录屏可能串台 | `controllers/_base.py::_ADBControllerBase.__init__`；`adblab/application/install_batch.py::InstallBatchUseCase`；`adblab/application/device_batch.py::DeviceBatchUseCase`；`adblab/application/screen_record.py::ScreenRecordUseCase` | 多设备或快速重复操作产生错误进度、错误路径、状态覆盖和内存增长 | Screenshot 走 OperationManager；安装批次走 InstallBatchUseCase；卸载/清数据/重启/当前 Activity 走 DeviceBatchUseCase；录屏走 ScreenRecordUseCase；通用 `_pending_ops` 已删除。Controller 的 `_batch_starts` 已增加 `_reject_concurrent_batch` 守卫，同类批次收口前拒绝新批次，避免按操作名覆盖串台 | Closed |
...
| Medium | `ADBApp.get_current_activity_async()` 在命令失败/无结果时仍可能返回 success | `models/adb_app.py::get_current_activity_async` | UI/上层把未知前台 Activity 当成功，掩盖断线或权限问题 | 已传播真实 CommandResult：success = 两次 dumpsys 命令均成功，失败附 error，无解析结果返回明确失败 | Closed |
| Medium | 录屏 pull 成功但远端清理失败时整体返回失败 | `models/adb_advanced.py::pull_recorded_video_async` | 用户误以为本地视频无效并重复操作；状态不精确 | 已分离 pull 与 cleanup 状态：pull 成功保留 local_path 并返回可选 cleanup_error，清理失败不再使整体失败 | Closed |
...
| Medium | 响应式重做后 `gui/main_frame.py` 曾约 2,500 行 | `gui/main_frame.py`（拆分前 2,489 行） | 组合根继续膨胀，信号接线/关闭清理难以审查，回归成本高 | 已拆出 `gui/main_frame_actions.py`（无 UI 动作）、`gui/pages/fluent_pages.py`（Gallery 页面）、`gui/secondary_windows.py`（二级窗口托管）和 `gui/close_controller.py`（异步关闭状态机）；旧工具栏模块已删除 | Closed |
...
| Low | 全量 pytest 偶发"测试全绿但进程退出异常"（退出挂起或 0xC0000005，约 10 次全量中出现 3 次）；另观测到 `tests/live_logcat_close_probe.py` 子进程在打印成功 JSON 后以 0xC0000374（堆损坏）退出，使 `test_continuous_logcat_close_stress_never_enters_main_window_exit_path` 报 `returncode != 0`（探针业务逻辑实际通过） | `pytest` 全量套件收尾阶段；`tests/live_logcat_close_probe.py` | 本地误报失败，浪费重跑时间 | 测试结果本身不受影响（进度条全绿 / 探针 JSON 成功）；怀疑为 Qt 延迟删除在解释器关闭期的环境级竞态（qfluentwidgets SmoothScrollDelegate/SmoothScrollArea 子进程关闭路径）；已复现最小化 MainFrame 与 TextEdit 单独退出均干净，建议后续用最小化探针定位后修复 | Open |
...
| Medium | 全局 `QThreadPool` 的 QRunnable 关闭时不统一等待；Controller `ThreadPoolExecutor(4)` 使用 `wait=False`，已经运行的长命令（install 120s、bugreport 180s）仍可能拖慢退出 | `models/adb_model.py::async_command/ADBModelCore.__init__`；`controllers/_base.py::_ADBControllerBase.shutdown` | 已运行任务可能持续到 timeout 上限 | 已通过 `long_pool` 分流长任务；2026-08-25 增加 model 终态准入栅栏，Controller 先栅栏再清理，未开始的 QRunnable 不再执行方法体；录屏与 Monkey/logcat 的 ProcessRunner 启停在同一锁内排序。仍待为已运行 QRunnable/executor 任务增加协作式取消和有界等待；应用级全局 tracked-process 兜底与 Remote 任务仍可并发，底层输入 Shell 顺序待实机确认 | Partial |
...
| High | 端点防护（火绒企业版 ESEndpoint，实测环境）对无签名打包 exe 的文件读写瞬时锁定/附加扫描块，`DeviceStore.load()` 曾失败即清空内存快照，打包应用启动后设备列表被清空且已存 WiFi 目标丢失 | `models/device_store.py::load/_read_text_with_retry/_parse_snapshot/_persist_snapshot`；`gui/main_frame.py::_ScanThread/_bootstrap_adb_async`；现场证据 `%LOCALAPPDATA%\ADBLab\config\connected_devices.yaml.corrupt-*`（4510/414 字节交替） | 打包环境下设备列表每次启动清空、无自动重连、`.corrupt` 备份堆积；每次 adb 进程启动被端点防护拖慢约 7 秒（冷启动 server 约 18 秒），原 5 秒扫描超时导致首轮扫描必失败 | 代码侧已止血：加载失败保留内存快照（只备份、不清空），读取按瞬态 OSError 重试，解析失败时截断 NUL 尾部扫描块只取首个 YAML 文档并回写规范化文件，写盘重试一次，失败原因走 UI 日志面板；bootstrap 后台预热 `start-server`（超时 30 秒）、设备扫描超时 15 秒；曾引入的"连续失败自动恢复 ADB Server"机制（`services/adb_recovery.py`、诊断信号、恢复阈值/冷却配置）已按产品决定整体移除，扫描失败仅更新发现状态；扫描调用改为 ProcessRunner + 100ms 轮询推进（停止请求立即终止在途 adb 子进程），修复关闭窗口时 QThread 在运行中被销毁的致命错误（`QThread: Destroyed while thread is still running`），冒烟探针验证干净退出；MobilePerf 内置的无签名 1.0.39 及 darwin/linux 平台二进制已移除（约 6.6MB），`get_adb_path()` 回退统一走 `utils.adb_resolver`，消除 39/41 协议互杀；测试 80+ 项通过。根治仍待：火绒控制台白名单/信任区或 exe 代码签名（需 IT/公司证书），授权实机复验打包产物 | Partial |

| High | 引入 qfluentwidgets（`PySide6-Fluent-Widgets` 1.11.3）作为 UI 依赖，其为 GPLv3 双许可（非商用 GPLv3、商用需付费） | `requirements.txt`；`gui/`（后续 Phase 1-5 迁移） | 若对外分发 exe 将触发 GPLv3 著佐权义务，ADBLab 须整体以 GPLv3 开源或向作者购买商用许可；违反"仅内部使用"边界会带来法律风险 | 常驻约束：仅内部使用、不对外分发；如需对外分发，先决策 GPLv3 开源或购买商用许可（qfluentwidgets.com/price） | Open |
| Low | UI 曾同时存在 qfluentwidgets、原生可见控件和 `gui/widgets/fluent/` 自研封装，组件状态/QSS/弹出列表契约彼此冲突 | 已删除的 `gui/widgets/fluent/`、`gui/styles/qss.py`；当前 `gui/styles/fluent.py`、`gui/widgets/preset_spin_box.py` | 同类控件行为不一致，主题与响应式修复需要维护多套实现 | 已完全删除旧控件封装和全局旧 QSS；通用可见控件直接使用 qfluentwidgets，设备地址历史改用 `EditableComboBox` 自带 `RoundMenu`；仅保留无继承的配置辅助和严格数值业务复合控件。Qt 布局、事件、信号、窗口与线程生命周期仍按参考项目的底层依赖保留 | Closed |
| Medium | 扫描或手动刷新失败曾可能用空列表覆盖 UI，恢复后相同集合也可能不再发布；慢速元数据任务可能跨设备拓扑写回 | `gui/main_frame.py::_ScanThread.invalidate_snapshot/_on_operation_completed`；`controllers/_device.py::_process_device_list/_async_update_devices` | 设备暂时不可用时丢失最后快照，状态卡在 unavailable，已离线设备被旧元数据重新写回 | 失败只标记 unavailable、不发送空列表；手动刷新失败使持续扫描快照失效，下一次成功即使集合相同也重发；Controller 以拓扑 generation 在写 DeviceStore 和二次发布前丢弃错代任务 | Closed |
| Low | qfluentwidgets 改版后曾残留旧壳、包装页、Settings 对话框和无调用控件 | 已删除的 `gui/main_frame_shell.py`、`gui/main_frame_toolbar.py`、`gui/dialogs/settings_dialog.py`、`gui/pages/devices_page.py`、`gui/pages/log_page.py`、`gui/widgets/double_click_button.py`，以及两个旧 Settings 测试 | 维护者可能修改不再运行的路径，包中携带失效代码，测试描述旧交互 | 2026-09-03 已连同 `gui/styles/qss.py`、`gui/widgets/fluent/` 一并清理；当前 UI 事实来源收敛到 MainFrame、`fluent_pages.py`、`main_frame_actions.py`、`styles/fluent.py` 与 qfluentwidgets。旧 splitter 配置键只保留 schema 兼容，不恢复旧布局 | Closed |
