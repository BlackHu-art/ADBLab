...
| Medium | 全量 pytest 约 930 项、约 11 分钟，门禁时长偏高 | `tests/`；`tests/test_responsive_panels.py` 等几何扫描文件 | 提交/CI 反馈慢，降低门禁执行意愿 | 已把响应式几何扫描防抖降到 1ms（单文件 6min→1.5min）；继续引入按层/模块 fast 子集与耗时预算 | Open |
