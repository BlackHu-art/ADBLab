...
| Medium | 全量 pytest 约 930 项、约 11 分钟，门禁时长偏高 | `tests/`；`tests/test_responsive_panels.py` 等几何扫描文件 | 提交/CI 反馈慢，降低门禁执行意愿 | 已把响应式几何扫描防抖降到 1ms（单文件 6min→1.5min）；已按 ADR-0003 Phase 0 引入 unit/ui/integration marker 与 CI `-m "not ui"` 快速子集（本地约 12 秒/367 项）；`wait_until` deadline 放宽到 6000ms 后全量 940 项顺序无关稳定通过、本机约 5 分钟；`test_model_execution.py` 拆分与按层耗时预算待 Phase 2 | Partial |
