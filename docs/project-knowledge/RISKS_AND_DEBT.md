...
| Medium | 响应式重做后 `gui/main_frame.py` 仍约 2,500 行 | `gui/main_frame.py`（2,489 行） | 组合根继续膨胀，信号接线/关闭清理难以审查，回归成本高 | 已抽出 `gui/screen_adapter.py`；继续按工具栏/工作区约束/关闭状态机拆分 | Open |
| Medium | 全量 pytest 约 930 项、约 11 分钟，门禁时长偏高 | `tests/`；`tests/test_responsive_panels.py` 等几何扫描文件 | 提交/CI 反馈慢，降低门禁执行意愿 | 已把响应式几何扫描防抖降到 1ms（单文件 6min→1.5min）；已按 ADR-0003 Phase 0 引入 unit/ui/integration marker 与 CI `-m "not ui"` 快速子集；`test_model_execution.py` 拆分与耗时预算待 Phase 2 | Partial |
...
| Low | ruff.toml 为 `mobileperf/**` 豁免 E402/UP031、`tests/live_logcat_close_probe.py` 豁免 E402 | `ruff.toml::[lint.per-file-ignores]` | 豁免区域脱离 lint 门禁，违规样式会继续积累 | MobilePerf 执行层重做（shell=True 重构）后移除 `mobileperf/**` 豁免；探针脚本结构变化后移除 E402 豁免 | Open |
| Low | Pillow、psutil 已锁定版本但一方源码未发现导入 | `requirements.txt`；导入扫描 | 构建面与供应链面略大于实际需要 | 确认间接使用或移除；评估清理 `mobileperf/setup.py` 遗留的 `requests` 描述 | 待确认 |
...
2. MainFrame 大文件拆分（screen_adapter 之后继续）与 Gate B2 异步关闭状态机。
