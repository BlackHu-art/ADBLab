...
| Low | qfluentwidgets 迁移残留少量原生 QSS（非缺陷，为"无等价物"的既定技术债；已用自研薄封装 DangerPushButton/ScalableListWidget/FluentStatusBar/ScalableGroupBox/FluentComboBox 收敛危险按钮、设备列表、状态栏、分组框与 editable 下拉框，状态徽标已用 InfoBadge 收敛，对话框页头/日志卡片/工具栏已用 CardWidget 收敛，标题/副标题/字段/内联/加粗标签已用 FluentLabel 收敛，文件表格/应用树/日志输出框已用 TableWidget/TreeView/TextEdit/PlainTextEdit 收敛，StrictIntSpinBox/预设按钮/严格整数下拉已用 qfluentwidgets SpinBox/ToolButton/EditableComboBox 收敛，日志级别过滤下拉已用 ComboBox 收敛，按钮 QSS 全部移除，滚动条已用 SmoothScrollArea/SmoothScrollDelegate 收敛并移除 SCROLLBAR_STYLE，应用详情页签已用 TabWidget 收敛并移除 QTabBar 焦点规则，全局文字色/背景/表头色已用 QPalette 收敛并移除 PANEL_BASE_STYLE） | `gui/styles/qss.py`（仅剩 `COMBO_BOX_STYLE` 供 FluentComboBox 自承载）；`gui/widgets/preset_spin_box.py` | 未来误判为"未迁移完成"而强行替换会破坏功能：设备地址下拉框依赖原生 QComboBox 的 setModel/setView 表格视图（品牌/型号/IP 三列），qfluentwidgets ComboBox/EditableComboBox 无该能力，故 FluentComboBox 与 COMBO_BOX_STYLE 保留 | 保持现状；如 qfluentwidgets 后续版本提供表格视图下拉再评估替换 | Open |
