...
- 语言只转换显示文案；主题枚举、缩放数值、字体默认空字符串、WorkspaceRoute 标识、远程控制
  预设及方向参数、应用类型和文件排序字段仍使用稳定值。三种应用词库同时处理历史中文与英文源文案，
  覆盖十二项主导航、设备栏、业务面板、功能页及其弹窗、状态、工具提示和无障碍名称。
  设备名称、包名、路径、用户输入及命令/日志原始输出保留原文；已知的应用错误模板在视图边界翻译，
  不修改模型或设备返回值。
