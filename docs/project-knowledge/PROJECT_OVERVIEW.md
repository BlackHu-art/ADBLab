...
ADBLab 是面向 Android 设备调试、应用测试和性能诊断的 PySide6 桌面工具。它把 ADB、scrcpy、logcat、dumpsys、Monkey 和移植版 MobilePerf 组织成图形化工作台，主入口为 `main.py::_run_gui()`，当前版本由 `utils/app_metadata.py::APP_VERSION` 定义为 3.2.0。
...
- 活跃版本：3.2.3；当前扫描分支 `dev`（自基线 `8b84f8d` 重做，提交清单见 [../README.md](../README.md)）。
