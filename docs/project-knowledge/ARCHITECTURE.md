...
- `gui/widgets/device_context_bar.py::DeviceContextBar` 位于主窗口内容布局中、页面堆叠外，功能页滚动时
  ...
  设备摘要以带图标的系统、屏幕、内存、存储四项组成，按 4/2/1 列重排；展开区补充品牌、型号、
...
- 功能切换由 `FluentWindow.navigationInterface` 的一级入口承担。展开模式显示图标与文字；
  ...
  不透明填充。设备栏与内容栈共用 `workspaceSurface`，云母生效时使用 Gallery 的明暗半透明底色、
...
- 截图与诊断页的 Monkey 分为测试目标与包信息、运行参数、事件分布、异常处理及执行状态。
  包名沿用页面唯一输入，包信息获取为只读操作，查询期间才显示取消；开始测试仍自动核对目标。
  目标和参数收拢为统一内边距的面板，包名只展示一次；参数采用上方标签与等宽字段网格，按宽度
  和字号重排。比例合计归属事件分布，执行状态直接派生自原任务状态。
- Performance 使用顶部运行操作区，采集计划与日志结果在宽屏并排、窄屏纵排；诊断选项可折叠，
  Monkey 参数按启用状态展开。计划与结果各自使用原生卡片，内部标题使用 UI 字号，字段帮助
  保留在提示与无障碍描述中。重排与折叠保留控件及原始输入，诊断数字校验失败先展开再定位。
  独立页面只有一个主滚动区，嵌入后由工作区承接滚动。日志与图表按各自字体测高，图表在采集
  结束后读取 CSV，坐标轴和图例跟随主题。
...
- Screenshot 通过 `prepare_for_workspace()` 隐藏内部页头的标题、说明与数量徽标，截图结果页只保留
