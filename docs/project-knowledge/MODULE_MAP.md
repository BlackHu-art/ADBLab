...
| 内嵌功能页 | App Manager、File Explorer、Live Logcat、Performance、Screenshot 与 Settings About 页面 | `gui/features/`、`gui/dialogs/app_manager*.py`、`file_explorer*.py`、`live_logcat*.py`、`performance_launcher*.py` | `test_app_manager_selection.py`、`test_screenshot_page.py`、`test_model_performance_launcher.py` |
| 瞬态交互与样式 | 消息、输入、短操作表单、系统文件选择器，以及主题、字体和复合控件 | `gui/dialogs/fluent_dialog.py`、`gui/styles/`、`gui/widgets/preset_spin_box.py` | `test_fluent_dialog_contract.py`、`test_fluent_components.py`、`test_feature_typography.py` |
