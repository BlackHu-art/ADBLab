...
| 文件与 Remote 服务 | 文件命令、页面串行传输与后台预览解码、scrcpy 多设备并发预检与会话启停、直连/持久输入和 Remote 生命周期 | `services/file_explorer.py`、`models/file_explorer_worker.py`、`gui/dialogs/file_explorer_transfers.py`、`gui/dialogs/file_explorer_preview_tasks.py`、`services/remote/` | `test_file_explorer_service.py`、`test_file_explorer_preview.py`、`test_remote_services.py`、`test_remote_sessions.py`、`test_remote_input_backend.py`、`test_remote_input_shutdown.py` |
