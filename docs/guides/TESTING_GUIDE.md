...
| 生命周期与探针 | `test_model_shutdown_admission.py`、`test_window_lifecycle.py`、`test_gui_startup.py`、`test_mainframe_startup.py`、`test_startup_splash.py`、`live_logcat_close_probe.py` | 关闭准入、启动首帧与中止、QObject 晚到回调、线程/进程释放；启动相关 Qt 测试归入 `ui` marker |
