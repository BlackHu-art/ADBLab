...
class MemInfoPackage(object):
    ...
    def _parse(self):
        """从进程级 dumpsys meminfo 输出中提取稳定的内存指标。

        不同 Android 版本的输出结构差异较大，此处优先提取较稳定且具有代表性的
        Total PSS、Java Heap、Native Heap 和 System 指标。
        """
...
class MemInfoDevice:
    ...
    def _parse(self):
        """解析并保存 Total RAM、Free RAM、Used RAM 及目标进程 PSS。"""
