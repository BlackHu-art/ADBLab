...
class FdInfoPackageCollector:
    ...
    def __init__(
        self, device, pacakgename, interval=1.0, timeout=24 * 60 * 60, fd_queue=None,
    ...
    def stop(self):
        ...
        worker = getattr(self, "collect_fd_thread", None)
        if worker is not None:
            worker.join(timeout=1)
            if not worker.is_alive():
                self.collect_fd_thread = None

    def get_process_fd(self, *, timeout: float = 10):
    ...
    def _collect_fd_thread(self, start_time):
        ...
        while not self._stop_event.is_set() and time.monotonic() < end_time:
            try:
                ...
                # 从目标进程状态中获取文件描述符容量。
                fd_pck_info = self.get_process_fd(timeout=min(10, end_time - time.monotonic()))
...
class FdMonitor:
    ...
    def __init__(
        self, device_id, packagename, interval=1.0, timeout=24 * 60 * 60, fd_queue=None,
        *, process_samples=None,
    ):
        ...
        self.fd_package_collector = FdInfoPackageCollector(
            self.device, packagename, interval, timeout, fd_queue, process_samples=process_samples,
        )
