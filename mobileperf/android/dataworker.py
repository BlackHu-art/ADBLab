        ...
        if isinstance(fps_data, dict):
            ...
            try:
                ...
                with open(self.fps_filename, "a+") as writer:
                    logger.debug(
                        "dataworker write fps data in  dataworker銆俧ps timestamp: "
                        + str(fps_data[0])
                    )
        ...
        if isinstance(mem_data, dict):
            ...
            try:
                ...
                with open(self.mem_filename, "a+") as writer:
                    logger.debug(
                        "write mem data in  dataworker銆傘€傘€傘€傘€傘€?mem timestamp: "
                        + str(mem_data[0])
                    )
        ...
        if isinstance(fd_data, dict):
            ...
            try:
                ...
                with open(self.fd_file, "a+") as writer:
                    logger.debug(
                        "write fd data in  dataworker銆傘€傘€傘€傘€傘€?fd timestamp: "
                        + str(fd_data[0])
                    )
    ...
    def _get_thread_save(self, thread_data, timestamp):
        if isinstance(thread_data, dict):
            ...
            try:
                ...
                dic = {
                    "time": thread_data[0] * 1000,
                    "package": thread_data[1],
                    "pid": thread_data[2],
                ...
                with open(self.thread_file, "a+") as writer:
                    logger.debug(
                        "write thread data in  dataworker銆傘€傘€傘€傘€傘€?thread timestamp: "
                        + str(thread_data[0])
                    )
        ...
        if isinstance(power_data, dict):
            ...
            try:
                ...
                with open(self.power_filename, "a+") as writer:
                    logger.debug(
                        "write power data in dataworker銆傘€傘€傘€傘€傘€?timestamp:"
                        + str(power_data[0])
                    )
