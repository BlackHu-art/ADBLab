...
- **应用管理 / 应用与诊断 / 截图与屏幕**：应用管理显示单设备应用列表；应用与诊断集中包管理、APK 工具、Monkey 和报告；截图与屏幕提供文本输入、截图录屏和截图浏览。
- **系统工具 / 实时 Logcat / 性能采集**：系统命令与设备配置在同一页面，其余工具独立直达。
- **操作结果**：任务中心的“本次操作”查看通用操作状态、正文和附件，“测试结果”保留 Monkey 与性能测试归档；Monkey 页显示运行状态，性能页提供日志和结果图表，设置页提供应用诊断导出。
- **设置**：界面语言、主题、字体、显示缩放、窗口、路径、日志、设备扫描配置及 ADB 执行环境维护；About 信息也位于设置页。

文件管理等业务页顶部显示设备栏，首页、设备概览、任务中心和设置页不显示。设备栏按当前功能
切换单选或多选；固定设备功能选择目标时同时切换会话。连接、刷新和“更多”中的断开操作位于
设备概览，设备信息在概览卡内展开。Monkey 开始前先获取所有目标设备的测试包信息。
设置中的显示缩放支持跟随系统及 100%～200%，重启后生效；
字号使用 pt，既有字号保持不变，可选择 11 pt 获得更紧凑的界面。
文件管理、已安装应用列表和实时 Logcat 按单设备上下文在窗口内创建、复用和关闭；性能采集按设备
维护独立会话，支持多设备并行和结束后的静态图表。Monkey 与性能采集支持参数方案和跨重启的测试
归档。截图结果使用独立的无设备会话。需要确认、短文本输入或系统文件选择时才使用临时窗口。
