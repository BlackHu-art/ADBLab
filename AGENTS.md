...
- 当前代码、工程配置和可执行测试是实现事实来源；它们与文档冲突时，先核实代码，再修正文档
  或把无法确认的结论标为“待确认”。
- 约束按单一来源维护：本文件负责协作、授权和修改边界；
  [测试指南](docs/guides/TESTING_GUIDE.md) 负责测试选择、测试规范和注释规范；
  [构建指南](docs/guides/BUILD_AND_RUN.md) 负责环境、打包与版本；
  [文档约定](docs/CONTRIBUTING_DOCS.md) 负责知识库维护。ADR 和 archive 中的阶段命令、
  “每步全量”和旧门禁只说明当时任务，不构成当前任务的执行要求。
...
- 只在任务明确涉及且来源已确认时修改 `resources/icons/`、`runtime-tools/`、
...
以下操作在当前会话尚未获得对应授权时，先说明选项、影响和推荐方案并等待用户确认：
删除文件或功能；破坏公共接口；改变用户
...
用户已明确授权清理失效内容时，可以移除已核实无消费者的内部代码和已失效的文档段落，记录
引用、替代入口及验证证据；不能只凭全文搜索无引用就删除 Qt 回调、反射入口、打包资源、兼容
迁移或有效回归测试。已有未提交修改仍须保留；历史材料有追溯价值时保留在归档区。

## 最小充分验证

- 默认先运行直接测试，再按调用链扩大到受影响模块。执行前说明“本轮修改边界 → 测试文件或
  节点 → 扩大范围的原因”；不能把整个脏工作树中此前已验收的修改重新视为本轮范围。
- 全量触发条件和重跑规则统一见
  [增量验证策略](docs/guides/TESTING_GUIDE.md#增量验证策略)。全量是代码稳定后的条件性验收，
  不是修改、修复测试失败、整理文档、完成长任务或推送 main 后的默认动作。未触发全量条件而
  未跑全量不属于验证缺失。多 agent 子任务只运行自己的直接与关联测试，由主任务决定集成范围。
...
- 完整门禁的唯一命令清单见
  [测试指南](docs/guides/TESTING_GUIDE.md#完整门禁命令)；打包检查按上面的实际触及边界选择，
  不把 pytest、完整构建和实机验收互相绑定。
