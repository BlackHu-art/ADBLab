...
class PerformanceLauncherForm:
    ...
    def _build_config_section(self, package_name: str) -> QWidget:
        ...
        target = self._section_card(tr("采集目标"), "performanceTarget")
        sampling = self._section_card(tr("采样与诊断"), "performanceSampling")
        output = self._section_card(tr("结果输出"), "performanceOutput")
        ...
        monkey.viewLayout.addWidget(
            self._field(
                "monkey", tr("随机操作压力测试"), self._frame.monkey_check, CONFIG_HINTS["monkey"]
            )
        )
