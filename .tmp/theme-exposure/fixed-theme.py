...
def apply_dark_title_bar(window: QWidget) -> None:
    ...
    from gui.window_effects import set_dwm_attribute

    set_dwm_attribute(int(window.winId()), 20, int(_current_theme == "Dark"))
...
_theme_switch_in_progress: bool = False
...
def _sync_qfluentwidgets_theme() -> None:
    ...
    setTheme({"Dark": Theme.DARK, "Light": Theme.LIGHT}[_current_theme])
    setThemeColor(_accent_color)


def _sync_native_theme(mode: str) -> str:
    """让 Qt 原生窗口与控件共用明暗模式；System 解除显式覆盖后再解析。

    只设置进程内的配色偏好，不修改 Windows 个性化配置。不支持覆盖的平台仍按
    显式选择绘制控件；原生窗口材质由窗口层判断是否可用。
    """
...
class ThemeMixin:
    ...
    def switch_theme(cls, name: str):
        """在 GUI 线程同步原生配色、主题状态和控件，再通知项目观察者。"""
        global _current_theme, _theme_mode, _theme_switch_in_progress
        # setColorScheme 可同步发出系统配色信号；外层切换已负责本次刷新，
        # 重入会重复刷新并可能在 System 解除覆盖时再次覆盖原生选择。
        if _theme_switch_in_progress:
            return
        aliases = {"Auto": "System", "System": "System", "Light": "Light", "Dark": "Dark"}
        mode = aliases.get(str(name), "System")
        _theme_switch_in_progress = True
        try:
            _theme_mode = mode
            _current_theme = _sync_native_theme(mode)
            # Fluent 的主题信号会同步刷新窗口。先提交主题与调色板，避免回调
            # 使用上一主题，把已切成深色的云母重新设置为浅色。
            _sync_application_palette()
            _sync_qfluentwidgets_theme()
            _theme_signal.changed.emit(mode)
        finally:
            _theme_switch_in_progress = False
