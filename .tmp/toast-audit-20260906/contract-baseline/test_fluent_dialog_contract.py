...
def test_fluent_message_static_result_owns_lifetime_with_upstream_auto_delete(
    qt_application, monkeypatch
):
    destroyed = []

    class UpstreamAutoDeleteMessageDialog(_FluentMessageDialog):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, True)
            self.destroyed.connect(lambda: destroyed.append(True))
            QTimer.singleShot(
                240, lambda: QTest.mouseClick(self.yesButton, Qt.MouseButton.LeftButton)
            )

    monkeypatch.setattr(
        fluent_dialog_module, "_FluentMessageDialog", UpstreamAutoDeleteMessageDialog
    )
    owner = QWidget()
    owner.resize(720, 540)
    owner.show()
    assert FluentMessageBox.warning(owner, "Warning", "Check input") == 1
    assert destroyed == []
    _flush_deferred_deletes(qt_application)
    assert destroyed == [True]
    owner.close()
