...
def test_dark_theme_updates_hidden_workspace_surfaces_before_navigation(qt_application, mica):
    """隐藏分区切换主题后首次打开时，不得露出创建期的浅色宿主。"""

    BaseStyles.switch_theme("Light")
    settings = _MainFrameSettings()
    settings.values["mica_enabled"] = mica
    ...
    try:
        ...
        layout_surfaces = [wrapper, wrapper.layout().itemAt(0).widget()]
        assert all(not widget.autoFillBackground() for widget in layout_surfaces)
        animation = frame.stackedWidget.view._ani
        animation.setCurrentTime(animation.duration())
        image = frame.grab().toImage()
        scale = image.devicePixelRatio()

        def blank_pixel(widget, point):
            position = widget.mapTo(frame, point)
            return image.pixelColor(round(position.x() * scale), round(position.y() * scale))

        material = blank_pixel(
            frame._content_surface, QPoint(frame._content_surface.width() - 3, 20)
        )
        assert all(blank_pixel(widget, QPoint(2, 2)) == material for widget in layout_surfaces)
...
def test_theme_switch_keeps_mica_content_stack_edges_borderless(qt_application, monkeypatch):
    """共用表面保留一像素外边框，内容栈不能在内部恢复第二层边框。"""
    ...
    def assert_edges_match_content_surface() -> None:
        image = frame.grab().toImage()
        ...
        scale = image.devicePixelRatio()
        # 第零像素属于 workspaceSurface 外框；检查其内侧没有新增边框或实色条。
        for edge, adjacent in (
            (QPoint(origin.x() + 1, left_y), QPoint(origin.x() + 3, left_y)),
            (QPoint(top_x, origin.y() + 1), QPoint(top_x, origin.y() + 3)),
        ):
    ...
    try:
        ...
        qt_application.processEvents()
        # 用可控背景验证 Qt 遮罩，真实桌面的 DWM 纹理仍需人工检查。
        if frame.isMicaEffectEnabled():
            frame.setBackgroundColor(QColor("#315879"))
        assert_edges_match_content_surface()
        ...
        for theme in ("Dark", "Light"):
            ...
            QTest.qWait(250)
            qt_application.processEvents()
            # Mica/DWM 稳定后，样式管理器也不能恢复内容栈自身的第二层边框。
            assert_edges_match_content_surface()
    finally:
        BaseStyles.switch_theme(original_theme)
        frame._unbind_window_screen()
...
def test_theme_switch_updates_mica_fluent_window_shell(qt_application):
    ...
    try:
        ...
        for theme in ("Light", "Dark"):
            ...
            expected = BaseStyles.color("WINDOW_BG").lower()
            assert all(
                surface.autoFillBackground() == (not frame.isMicaEffectEnabled())
...
def test_screenshot_batch_updates_inline_media_without_stealing_navigation(
    ...
    try:
        ...
        assert args == (frame, "截图已完成", "结果已加入“截图结果”页面。")
...
def test_windows_apps_page_responsive_groups_fit_at_supported_widths(
    ...
    try:
        ...
        # 基础参数两组，事件分布和异常选项各三组；每组自身有序且贴齐左边界，
        # 不再要求不同语义分组使用相同列宽，合计归属事件分布标题。
        for starts in default_starts:
            assert starts[0] == 0
        ...
        assert app_panel._pct_total_lbl not in app_panel.monkey_parameter_binding.widgets()
