...
class PerformanceProgress(QWidget):
    ...
    def heightForWidth(self, width: int) -> int:
        ...
        text_height = sum(
            max(label.sizeHint().height(), label.heightForWidth(width))
    ...
    def refresh(self, state: str, *, active: bool) -> None:
        ...
        self.icon.setIcon(icon.icon(color=QColor(BaseStyles.color(color_key))))
    ...
    def _sync_animation(self) -> None:
        ...
        if self._busy and self._animation_enabled and self.isVisible():
            if not running:
