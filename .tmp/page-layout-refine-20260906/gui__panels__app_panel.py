...
class AppPanel(BasePanel):
    ...
    def build_ui(self) -> QWidget:
        ...
        gm_l.addWidget(self.monkey_target_heading)
        ...
        gm_l.addWidget(self.monkey_target_summary)

        self.monkey_package_card = SimpleCardWidget(g_m)
        package_info_layout = QVBoxLayout(self.monkey_package_card)
        package_info_layout.setContentsMargins(14, 12, 14, 12)
        package_info_layout.setSpacing(10)
        ...
        self.monkey_prepare_actions = self._add_monkey_actions(
            package_info_layout, self.monkey_get_package_btn, self.monkey_cancel_prepare_btn,
        )
        ...
        gm_l.addWidget(self.monkey_parameters_heading)
        ...
        # 不再占用第三组的标签列。
        label_width = (
            QFontMetrics(BaseStyles.font_for_role(FontRole.UI)).horizontalAdvance(
                tr("应用切换：")
            ) + 4
        )
        ...
        gm_l.addWidget(self.monkey_exceptions_heading)
        ...
        gm_l.addWidget(self.monkey_exceptions_hint)
        self._add_responsive_row(
            gm_l,
        ...
        self.monkey_run_actions = self._add_monkey_actions(
            gm_l, self.start_monkey_btn, self.kill_monkey_btn,
        )
        gm_l.addWidget(self.monkey_run_status)
    ...
    @staticmethod
    def _add_monkey_actions(layout, *buttons) -> QWidget:
        """按按钮自然宽度换行；隐藏的取消操作不占用动作区空间。"""

        row = QWidget()
        row.setMinimumWidth(0)
        flow = FlowLayout(row, isTight=True)
    ...
    def on_monkey_preparation_finished(self, request_id: str, result: dict) -> None:
        ...
        lines = [tr("测试包：{package}").format(package=package)]
