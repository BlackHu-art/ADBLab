...
class _PerformanceColumns(QWidget):
    ...
    def _reflow(self) -> None:
        ...
        self._grid.addWidget(self._configuration, 0, 0, Qt.AlignmentFlag.AlignTop)
...
class PerformanceLauncherForm:
    ...
    def _build_ui(self, package_name: str):
        ...
        results.addWidget(_PerformanceRow(
    ...
    def _field(self, key: str, title: str, field: QWidget, hint: str) -> QWidget:
        ...
        if hint:
            ...
            help_label.setAccessibleName(tr('{value0}说明').format(value0=title))
    ...
    def _build_config_section(self, package_name: str) -> QWidget:
        ...
        diagnostics.setContentsMargins(0, 8, 0, 0)
        ...
        monkey = self._section_card(tr("Monkey 压力测试"), "performanceMonkey")
        for section in (target, self._frame._diagnostic_tools, monkey):
        ...
        monkey.viewLayout.addWidget(self._frame.monkey_check)
        monkey_hint = apply_label_role(
            BodyLabel(tr(CONFIG_HINTS["monkey"])), FontRole.UI, color_key="TEXT_SECONDARY"
        )
        monkey_hint.setObjectName("configHint")
        monkey_hint.setWordWrap(True)
        self._apply_hint(self._frame.monkey_check, tr(CONFIG_HINTS["monkey"]))
        monkey.viewLayout.addWidget(monkey_hint)
        self._frame._monkey_details = self._build_monkey_row()
        monkey.viewLayout.addWidget(self._frame._monkey_details)
        self._frame._configuration_sections = (target, self._frame._diagnostic_tools, monkey)
