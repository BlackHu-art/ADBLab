...
from PySide6.QtCore import Qt, QThread, Signal, QSortFilterProxyModel, QSize
from PySide6.QtGui import QStandardItemModel, QStandardItem, QFont, QIcon, QColor, QPixmap, QPainter
from PySide6.QtWidgets import (
    QDialog, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QTextEdit,
    QTreeView, QHeaderView, QMessageBox, QFileDialog, QLineEdit, QMenu,
    QDialogButtonBox, QComboBox, QTabWidget, QListWidget, QListWidgetItem,
...
class AppManagerWorker(QThread):
    ...
    def run(self):
        ops = {
            ...
            "modify_app": lambda: self._modify_app(self.kwargs.get('action'), self.kwargs.get('package_name')),
            "backup_app": lambda: self._backup_app(self.kwargs.get('package_name'), self.kwargs.get('save_dir')),
            "restore_apps": lambda: self._restore_apps(self.kwargs.get('file_paths', [])),
            "modify_permission": lambda: self._modify_permission(
                self.kwargs.get('package_name'), self.kwargs.get('permission'), self.kwargs.get('action')),
    ...
    def _adb(self, *args, timeout=30):
        ...
        return subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                              text=True, creationflags=cf, timeout=timeout)
    ...
    def _load_apps(self):
        ...
        try:
            ...
            for line in r.stdout.splitlines():
                ...
                if not pkg: continue
                if "/data/app/" in fp: atype = "User"
                elif "/system/" in fp: atype = "System"
                elif "/vendor/" in fp: atype = "Vendor"
                else: atype = "Other"
    ...
    def _load_detail_batch(self, packages):
        ...
        for i, pkg in enumerate(packages):
            try:
                ...
                self.app_detail_batch.emit(pkg, label or pkg.split('.')[-1],
                                           f"{vn} ({vc})" if vn else "", itime)
            except Exception:
    ...
    def _fetch_app_details(self, pkg):
        ...
        self.app_details_loaded.emit({
            ...
            "Min SDK": m_ms.group(1) if m_ms else "?", "Target SDK": m_ts.group(1) if m_ts else "?",
        })
    ...
    def _fetch_permissions(self, pkg):
        ...
        def ps(h, t):
            m = re.search(h + r':\n((?:.+?\n)+?)(?:\n\S|\Z)', t, re.MULTILINE)
            return [l.strip() for l in m.group(1).strip().splitlines() if l.strip()] if m else []
        ...
        for line in ps(r'runtime permissions', out):
            ...
            if m: runtime.append((m.group(1).strip(), m.group(2) == "true"))
        self.permissions_loaded.emit(declared, requested, runtime)

    def _modify_app(self, action, pkg):
        cmds = {"disable": ["shell","pm","disable-user","--user","0",pkg],
        ...
        if r.returncode == 0: self.operation_done.emit(action)
    ...
    def _backup_app(self, pkg, save_dir):
        ...
        paths = [l.replace("package:","").strip() for l in r.stdout.strip().splitlines() if l.strip()]
        if not paths: self.log_message.emit(f"No APK for {pkg}"); return
        ...
        for i, zp in enumerate(files):
            ...
            try:
                with tempfile.TemporaryDirectory(prefix=f"rs_{app}_") as tmp:
                    ...
                    apks = [os.path.join(r,f) for r,_,fs in os.walk(tmp) for f in fs if f.endswith(".apk")]
                    ...
                    if is_split:
                        ...
                        for a in apks: self._adb("install", "-r", a, timeout=120)
                self.log_message.emit(f"Restored ({i+1}/{len(files)}): {os.path.basename(zp)}")
            except Exception as e: self.log_message.emit(f"Error: {e}")
...
class AppSortProxy(QSortFilterProxyModel):
    ...
    def lessThan(self, left, right):
        col = left.column(); ld = self.sourceModel().data(left); rd = self.sourceModel().data(right)
        if col == 2 and ld in self.STATUS_ORDER: return self.STATUS_ORDER[ld] < self.STATUS_ORDER[rd]
        if col == 3 and ld in self.TYPE_ORDER: return self.TYPE_ORDER[ld] < self.TYPE_ORDER[rd]
...
class AppDetailsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.detail_text.setFont(QFont("Consolas", 9)); dl.addWidget(self.detail_text)
        ...
        pb = QHBoxLayout()
        self.grant_btn = QPushButton("Grant Selected"); self.revoke_btn = QPushButton("Revoke Selected")
        self.grant_btn.clicked.connect(lambda: self._mp("grant"))
        self.revoke_btn.clicked.connect(lambda: self._mp("revoke"))
        pb.addWidget(self.grant_btn); pb.addWidget(self.revoke_btn); pl.addLayout(pb)
        self.tabs.addTab(pw, "Permissions")
        layout.addWidget(self.tabs)
        close_btn = QPushButton("Close"); close_btn.clicked.connect(self.close); layout.addWidget(close_btn)

    def _ps(self, parent, title):
        hl = QHBoxLayout(); hl.addWidget(QLabel(title))
        sb = QPushButton("Select All/None"); sb.setFixedSize(130, 28); hl.addWidget(sb)
        parent.addLayout(hl)
        lw = QListWidget(); lw.setMinimumHeight(100)
        lw.setSelectionMode(QListWidget.SelectionMode.MultiSelection); parent.addWidget(lw)
    ...
    def _ta(lw):
        ...
        for i in range(lw.count()): lw.item(i).setCheckState(st)
    ...
    def _od(self, d):
        ...
        for k, v in d.items(): self.detail_text.append(f"<b>{k}:</b> {v}")

    def _op(self, declared, requested, runtime):
        def fill(lw, items, fmt=lambda x: x):
            lw.clear()
            for item in items:
                i = QListWidgetItem(fmt(item)); i.setFlags(i.flags() | Qt.ItemFlag.ItemIsUserCheckable)
                i.setCheckState(Qt.CheckState.Unchecked); lw.addItem(i)
        fill(self.declared_list, declared); fill(self.requested_list, requested)
        fill(self.runtime_list, runtime, lambda r: f"{r[0]} (Granted: {r[1]})")

    def _mp(self, action):
        rc = [self.runtime_list.item(i).text().split(" (")[0] for i in range(self.runtime_list.count()) if self.runtime_list.item(i).checkState() == Qt.CheckState.Checked]
        rq = [self.requested_list.item(i).text() for i in range(self.requested_list.count()) if self.requested_list.item(i).checkState() == Qt.CheckState.Checked]
        sel = rc + rq
        if not sel: QMessageBox.warning(self, "No Selection", f"No permissions selected to {action}."); return
        for perm in sel:
            self._rw("modify_permission", package_name=self.package_name, permission=perm, action=action, operation_done=lambda _: self._rp())

    def _rw(self, op, **kw):
        w = AppManagerWorker(self.device_ip, op, **kw); w.setParent(self)
        for s in ["log_message","app_details_loaded","permissions_loaded","operation_done"]:
            if s in kw: getattr(w, s).connect(kw[s])
        if hasattr(self.parent(), 'log'): w.log_message.connect(self.parent().log)
        self._workers.append(w); w.start()

    def closeEvent(self, event):
        for w in self._workers: w.quit(); w.wait(1000)
...
class AppManagerDialog(QDialog):
    ...
    def _init_ui(self):
        layout = QVBoxLayout(self); layout.setSpacing(4); layout.setContentsMargins(8, 8, 8, 6)

        # Search + filter + view toggle
        top = QHBoxLayout(); top.setSpacing(6)
        top.addWidget(QLabel("Search:"))
        self.search_input = QLineEdit(); self.search_input.setPlaceholderText("Filter...")
        self.search_input.textChanged.connect(self._filter); top.addWidget(self.search_input, 1)
        top.addWidget(QLabel("Type:"))
        self.type_filter = QComboBox(); self.type_filter.addItems(["All", "User Apps", "System Apps"])
        self.type_filter.currentIndexChanged.connect(self._filter); top.addWidget(self.type_filter)
        ...
        self.refresh_btn.setFixedHeight(28); top.addWidget(self.refresh_btn)
        ...
        self.model.setHorizontalHeaderLabels(["", "App Name", "Package Name", "Version", "Status", "Type"])
        self.proxy = AppSortProxy(); self.proxy.setSourceModel(self.model)
        self.proxy.setFilterCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
        self.proxy.setFilterKeyColumn(-1)
        self.tree = QTreeView(); self.tree.setModel(self.proxy); self.tree.setSortingEnabled(True)
        self.tree.setEditTriggers(QTreeView.EditTrigger.NoEditTriggers)
        self.tree.setAlternatingRowColors(True); self.tree.setRootIsDecorated(False)
        self.tree.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.tree.customContextMenuRequested.connect(self._context_menu)
        self.tree.clicked.connect(self._on_row_clicked)
        h = self.tree.header(); h.setSectionResizeMode(0, QHeaderView.ResizeMode.Fixed)
        for i in range(1, 6): h.setSectionResizeMode(i, QHeaderView.ResizeMode.Interactive)
        self.tree.setColumnWidth(0, 40); self.tree.setColumnWidth(1, 160)
        self.tree.setColumnWidth(2, 320); self.tree.setColumnWidth(3, 100)
        self.tree.setColumnWidth(4, 70); self.tree.setColumnWidth(5, 60)
        ...
        self.icon_list.setIconSize(QSize(48, 48)); self.icon_list.setSpacing(4)
        ...
        btn_h = 30
        a1 = QHBoxLayout(); a1.setSpacing(4)
        labels_actions = [
            ("Uninstall Selected", "uninstall"), ("Disable Selected", "disable"),
            ("Enable Selected", "enable"), ("Deselect All", None),
        ]
        for t, a in labels_actions:
            b = QPushButton(t); b.setFixedHeight(btn_h)
            if a: b.clicked.connect(lambda _, act=a: self._modify_selected(act))
            else: b.clicked.connect(self._deselect_all)
            a1.addWidget(b, 1)
        layout.addLayout(a1)

        a2 = QHBoxLayout(); a2.setSpacing(4)
        for t, fn in [("Create Preset", self._create_preset), ("Load Preset", self._load_preset),
                       ("Backup Selected", self._backup_selected), ("Restore Backup", self._restore_apps),
                       ("App Details", self._show_details)]:
            b = QPushButton(t); b.setFixedHeight(btn_h); b.clicked.connect(fn); a2.addWidget(b, 1)
        layout.addLayout(a2)

        # Log (no label)
        self.log_output = QTextEdit(); self.log_output.setReadOnly(True)
        self.log_output.setMaximumHeight(100); self.log_output.setFont(QFont("Consolas", 9))
        self.log_output.setPlaceholderText("Operation log...")
        layout.addWidget(self.log_output)

        # Status
        self.status_bar = QStatusBar(); self.status_bar.showMessage("Ready"); layout.addWidget(self.status_bar)

    def _apply_theme(self, _name=""):
        bs = BaseStyles
        self.setStyleSheet(bs.PANEL_BASE_STYLE())
        bg = bs.color('INPUT_BG'); fg = bs.color('TEXT_PRIMARY'); border = bs.color('BORDER_COLOR')
        self.log_output.setStyleSheet(f"background-color:{bs.color('LOG_BACKGROUND')}; color:{bs.color('LOG_TEXT_COLOR')}; border:1px solid {border}; border-radius:{bs.RADIUS_MD}px;")
        self.tree.setStyleSheet(f"QTreeView {{ background-color:{bg}; color:{fg}; border:1px solid {border}; border-radius:{bs.RADIUS_MD}px; alternate-background-color:{bs.color('INPUT_BG_HOVER')}; }} QTreeView::item:selected {{ background-color:{bs.color('SELECTION_BG')}; color:{bs.color('SELECTION_TEXT')}; }} QHeaderView::section {{ background-color:{bs.color('BUTTON_BG')}; color:{fg}; padding:4px; border:1px solid {border}; }}")
        self.icon_list.setStyleSheet(f"QListWidget {{ background-color:{bg}; color:{fg}; border:1px solid {border}; border-radius:{bs.RADIUS_MD}px; }} QListWidget::item:selected {{ background-color:{bs.color('SELECTION_BG')}; color:{bs.color('SELECTION_TEXT')}; border-radius:4px; }}")
        self.status_bar.setStyleSheet(f"QStatusBar {{ color:{fg}; border-top:1px solid {border}; }}")
    ...
    def _load_apps(self):
        ...
        w.log_message.connect(self.log); w.apps_loaded.connect(self._populate)
    ...
    def _populate(self, apps):
        ...
        for name, pkg, st, at in apps:
            cb = QStandardItem(); cb.setCheckable(True)
            self.model.appendRow([cb, QStandardItem(name), QStandardItem(pkg),
                                  QStandardItem(""), QStandardItem(st), QStandardItem(at)])
        ...
        for name, pkg, st, at in sorted_apps:
            ...
            if st == "Disabled": item.setForeground(QColor("#999999"))
    ...
    def _on_detail(self, pkg, label, version, itime):
        ...
        for r in range(self.model.rowCount()):
            if self.model.item(r, 2).text() == pkg:
                if label: self.model.item(r, 1).setText(label)
                if version: self.model.item(r, 3).setText(version)
                break
    ...
    def _icon_double_click(self, item):
        pkg = item.data(Qt.UserRole)
        if pkg: self._show_details_for(pkg)
    ...
    def _filter(self):
        ...
        if ft == "User Apps":
            ...
            if text: self.proxy.setFilterRegularExpression(f"(?=.*{re.escape(text)})(?=.*User)")
        elif ft == "System Apps":
            self.proxy.setFilterFixedString("System")
            self.proxy.setFilterKeyColumn(5)
            if text: self.proxy.setFilterRegularExpression(f"(?=.*{re.escape(text)})(?=.*System)")
        else:
            if text: self.proxy.setFilterRegularExpression(text)
            else: self.proxy.setFilterRegularExpression("")
            self.proxy.setFilterKeyColumn(-1)
        ...
        for i in range(self.icon_list.count()):
            ...
            type_match = (ft == "All" or
                         (ft == "User Apps" and "User" in (item.toolTip() or "")) or
                         (ft == "System Apps" and "System" in (item.toolTip() or "")))
    ...
    def _on_row_clicked(self, index):
        ...
        if row < 0: return
        cb = self.model.item(row, 0); pkg = self.model.item(row, 2).text()
        if index.column() > 0:
            ns = Qt.CheckState.Unchecked if cb.checkState() == Qt.CheckState.Checked else Qt.CheckState.Checked
            cb.setCheckState(ns)
        (self.selected_packages.add(pkg) if cb.checkState() == Qt.CheckState.Checked else self.selected_packages.discard(pkg))

    def _context_menu(self, pos):
        idx = self.tree.indexAt(pos)
        if not idx.isValid(): return
        src = self.proxy.mapToSource(idx); row = src.row()
    ...
    def _show_details_for(self, pkg):
        dlg = AppDetailsDialog(self, self.device_ip, pkg); dlg.show()

    def _launch(self, pkg):
        w = AppManagerWorker(self.device_ip, "launch_app", package_name=pkg)
        w.log_message.connect(self.log); self._track_worker(w); w.start()
    ...
    def _modify_one(self, action, pkg):
        if action == "force_stop":
            ...
            self._track_worker(w); w.start()
            # Actually force-stop
            r = subprocess.run(["adb", "-s", self.device_ip, "shell", "am", "force-stop", pkg],
    ...
    def _backup_one(self, pkg):
        ...
        w.log_message.connect(self.log); w.backup_progress.connect(lambda p, m: self.log(f"[{p}] {m}"))
        self._track_worker(w); w.start()

    def _deselect_all(self):
        for r in range(self.model.rowCount()): self.model.item(r, 0).setCheckState(Qt.CheckState.Unchecked)
    ...
    def _get_selected_pkgs(self):
        if self._view_mode:
            return [it.data(Qt.UserRole) for it in self.icon_list.selectedItems() if it.data(Qt.UserRole)]
        return list(self.selected_packages)

    def _modify_selected(self, action):
        pkgs = self._get_selected_pkgs()
        if not pkgs: QMessageBox.warning(self, "None", "No apps selected."); return
        for pkg in pkgs:
            w = AppManagerWorker(self.device_ip, "modify_app", action=action, package_name=pkg)
            w.log_message.connect(self.log); self._track_worker(w); w.start()
        self._load_apps()
    ...
    def _restore_apps(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Select Backup ZIP(s)", "", "ZIP Files (*.zip)")
        if not files: return
        w = AppManagerWorker(self.device_ip, "restore_apps", file_paths=files)
        w.log_message.connect(self.log); w.backup_progress.connect(lambda p, m: self.log(f"[{p}] {m}"))
        w.operation_done.connect(lambda _: self._load_apps()); self._track_worker(w); w.start()

    def _show_details(self):
        if self._view_mode:
            items = self.icon_list.selectedItems()
            if not items: QMessageBox.warning(self, "None", "No app selected."); return
            pkg = items[0].data(Qt.UserRole)
        else:
            idxs = self.tree.selectedIndexes()
            if not idxs: QMessageBox.warning(self, "None", "No app selected."); return
    ...
    def _create_preset(self):
        ...
        dlg = QDialog(self); dlg.setWindowTitle("Create Preset"); dlg.setFixedSize(380, 280)
        l = QVBoxLayout(dlg)
        l.addWidget(QLabel("Preset Name:")); ni = QLineEdit(); l.addWidget(ni)
        l.addWidget(QLabel("Author (optional):")); ai = QLineEdit(); l.addWidget(ai)
        l.addWidget(QLabel("Description (optional):")); di = QTextEdit(); di.setMaximumHeight(60); l.addWidget(di)
        btns = QDialogButtonBox(QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
        btns.accepted.connect(dlg.accept); btns.rejected.connect(dlg.reject); l.addWidget(btns)
        if not dlg.exec(): return
        name = ni.text().strip() or "New Preset"
        data = {"name": name, "author": ai.text().strip(), "description": di.toPlainText().strip(),
    ...
    def _load_preset(self):
        ...
        if not pkgs: self.log("Preset empty."); return
        self._deselect_all()
        for r in range(self.model.rowCount()):
