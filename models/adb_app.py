...
class ADBApp(ADBModelCore):
        ...
        try:
            ...
            if not current_focus_line:
                return {"success": False, "device_ip": device_ip,
                        "error": "No mCurrentFocus found"}

            match = re.search(r"mCurrentFocus=Window\{.*?\s(\S+?)/(\S+)\}",
                             current_focus_line)
            if match:
                return {
                    "success": True,
    ...
    def install_apk_async(self, device_ip: str, apk_path: str,
        ...
        try:
            ...
            return {"success": True, "device_ip": device_ip,
                    "apk_path": apk_path, "output": result.stdout.strip(),
                    "index": idx, "apk_name": apk_name}
    ...
    @async_command
    def uninstall_app_async(self, device_ip: str, package_name: str,
                            idx: int) -> dict:
        try:
            result = subprocess.run(
            ...
            return {"success": True, "output": result.stdout.strip(),
                    "device_ip": device_ip, "package_name": package_name,
                    "index": idx}
        except subprocess.TimeoutExpired as e:
    ...
    def clear_app_data_async(self, device_ip: str, package_name: str, idx: int):
        try:
            ...
            return {"success": True, "device_ip": device_ip,
                    ...
                    "output": result.stdout.strip(), "index": idx}
        except Exception as e:
            return {"success": False, "device_ip": device_ip,
                    "package_name": package_name, "output": str(e), "index": idx}

    @async_command
    def restart_app_async(self, device_ip: str, package_name: str, index: int):
        try:
            stop_cmd = ["adb", "-s", device_ip, "shell", "am", "force-stop",
                       package_name]
            stop_output = self._execute_command(stop_cmd)

            start_cmd = ["adb", "-s", device_ip, "shell", "monkey", "-p",
            ...
            return {"success": False, "device_ip": device_ip,
                    "package_name": package_name, "output": e.output,
                    "index": index}
        except Exception as e:
            return {"success": False, "device_ip": device_ip,
                    "package_name": package_name, "output": str(e),
                    "index": index}
    ...
    def get_current_activity_async(self, device_ip: str, index: int = 0) -> dict:
        try:
            ...
            resumed_cmd = ["adb", "-s", device_ip, "shell", "dumpsys", "activity",
                          "activities"]
            ...
            return {"success": True, "device_ip": device_ip, "index": index,
                    "current_focus": current_focus,
                    "resumed_activity": resumed_activity}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "index": index,
                    "error": str(e)}
    ...
    def input_text_async(self, device_ip: str, text: str) -> dict:
        try:
            ...
            if result.startswith(("Error:", "Timeout:", "SystemError:")):
                return {"success": False, "device_ip": device_ip,
                        "error": result, "text": text}
            return {"success": True, "device_ip": device_ip, "text": text,
                    "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip,
    ...
    def list_installed_packages_async(self, device_ip: str, index: int) -> dict:
        try:
            ...
            return {"device_ip": device_ip, "success": True,
                    "packages": packages, "index": index}
        except subprocess.CalledProcessError as e:
            return {"device_ip": device_ip, "success": False,
                    "message": e.output, "index": index}
