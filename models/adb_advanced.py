...
class ADBAdvanced(ADBModelCore):
    ...
    @async_command
    def start_screen_record_async(self, device_ip: str, save_dir: str,
                                  duration: int = 180, width: str = "",
                                  height: str = "", bitrate: str = "8000000",
                                  ) -> dict:
        try:
            timestamp = datetime.now().strftime("%H%M%S")
            filename = f"record_{timestamp}.mp4"
            remote_path = f"/sdcard/{filename}"
            cmd = ["adb", "-s", device_ip, "shell", "screenrecord",
            ...
            return {"success": True, "device_ip": device_ip,
                    "remote_path": remote_path, "proc_pid": proc.pid,
    ...
    @async_command
    def pull_recorded_video_async(self, device_ip: str, remote_path: str,
                                  save_dir: str, filename: str) -> dict:
        try:
            ...
            self._execute_command(
                ...
                timeout=60)
            self._execute_command(
                ["adb", "-s", device_ip, "shell", "rm", remote_path])
            return {"success": True, "device_ip": device_ip,
    ...
    def input_tap_async(self, device_ip: str, x: int, y: int) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "input", "tap",
                 str(x), str(y)])
    ...
    def input_longpress_async(self, device_ip: str, keycode: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "input", "keyevent",
                 "--longpress", keycode])
            return {"success": True, "device_ip": device_ip, "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    @async_command
    def logcat_filtered_async(self, device_ip: str, log_path: str,
                              buffer: str = "main", priority: str = "V",
                              tag_filter: str = "", regex: str = "",
                              max_lines: str = "") -> dict:
        try:
            ...
            with open(log_path, 'w', encoding='utf-8') as f:
                f.write(log_content)
            return {"success": True, "device_ip": device_ip,
                    "log_path": log_path, "line_count": len(log_content.splitlines())}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    def forward_port_async(self, device_ip: str, local_port: str,
                           remote_port: str, protocol: str = "tcp") -> dict:
        try:
    ...
    @async_command
    def reverse_port_async(self, device_ip: str, remote_port: str,
                           local_port: str, protocol: str = "tcp") -> dict:
        try:
    ...
    @async_command
    def settings_list_async(self, device_ip: str,
                            namespace: str = "system") -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "settings", "list",
    ...
    @async_command
    def settings_get_async(self, device_ip: str, namespace: str,
                           key: str) -> dict:
        try:
            result = self._execute_command(
    ...
    def reboot_mode_async(self, device_ip: str, mode: str) -> dict:
        try:
            if mode == "system":
                self._execute_command(
                    ["adb", "-s", device_ip, "reboot"], timeout=3)
            elif mode in ("bootloader", "recovery", "fastboot"):
                self._execute_command(
                    ["adb", "-s", device_ip, "reboot", mode], timeout=3)
    ...
    @async_command
    def shell_rm_async(self, device_ip: str, path: str,
    ...
    @async_command
    def push_file_async(self, device_ip: str, local_path: str,
    ...
    @async_command
    def pull_file_async(self, device_ip: str, remote_path: str,
                        local_path: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "pull", remote_path, local_path],
    ...
    @async_command
    def grant_permission_async(self, device_ip: str, package: str,
                               permission: str) -> dict:
        try:
            result = self._execute_command(
    ...
    @async_command
    def revoke_permission_async(self, device_ip: str, package: str,
                                permission: str) -> dict:
        try:
            result = self._execute_command(
    ...
    def reset_permissions_async(self, device_ip: str, package: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "pm", "reset-permissions",
                 package])
            return {"success": True, "device_ip": device_ip,
                    "package": package, "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}

    @async_command
    def list_permissions_async(self, device_ip: str,
                               package: str = "") -> dict:
        try:
            if package:
                result = self._execute_command(
    ...
    @async_command
    def disable_package_user_async(self, device_ip: str,
                                   package: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "pm", "disable-user",
                 package])
            return {"success": True, "device_ip": device_ip,
                    "package": package, "output": result}
        except Exception as e:
    ...
    @async_command
    def send_broadcast_async(self, device_ip: str, action: str,
                             extras: dict = None) -> dict:
        try:
            cmd = ["adb", "-s", device_ip, "shell", "am", "broadcast",
                   "-a", action]
            if extras:
    ...
    def tcpip_mode_async(self, device_ip: str, port: str = "5555") -> dict:
        try:
            ...
            return {"success": True, "device_ip": device_ip,
                    "port": port, "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}

    @async_command
    def pair_device_async(self, ip_address: str, port: str,
                          pairing_code: str) -> dict:
        try:
            result = self._execute_command(
    ...
    def kill_process_async(self, device_ip: str, pid: str) -> dict:
        try:
            ...
            return {"success": True, "device_ip": device_ip, "pid": pid,
                    "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    def content_query_async(self, device_ip: str, uri: str,
                            ...
                            sort: str = "") -> dict:
        try:
            cmd = ["adb", "-s", device_ip, "shell", "content", "query",
                   "--uri", uri]
            if projection:
    ...
    @async_command
    def content_insert_async(self, device_ip: str, uri: str,
                             binds: dict = None) -> dict:
        try:
            cmd = ["adb", "-s", device_ip, "shell", "content", "insert",
                   "--uri", uri]
    ...
    @async_command
    def content_delete_async(self, device_ip: str, uri: str,
                             where: str = "") -> dict:
        try:
            cmd = ["adb", "-s", device_ip, "shell", "content", "delete",
                   "--uri", uri]
            if where:
                cmd.extend(["--where", where])
            result = self._execute_command(cmd, timeout=15)
    ...
    @async_command
    def cmd_bluetooth_enable_async(self, device_ip: str,
                                   enable: bool) -> dict:
        try:
            action = "enable" if enable else "disable"
            self._execute_command(
    ...
    def cmd_statusbar_expand_async(self, device_ip: str) -> dict:
        try:
            self._execute_command(
                ["adb", "-s", device_ip, "shell", "cmd", "statusbar",
                 "expand-settings"])
            return {"success": True, "device_ip": device_ip}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    def cmd_uimode_night_async(self, device_ip: str, enable: bool) -> dict:
        try:
            ...
            self._execute_command(
                ["adb", "-s", device_ip, "shell", "cmd", "uimode", "night",
                 mode])
            return {"success": True, "device_ip": device_ip, "mode": mode}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    @async_command
    def cmd_dumpsys_service_async(self, device_ip: str,
                                  service: str = "") -> dict:
        try:
            if service:
                result = self._execute_command(
                    ["adb", "-s", device_ip, "shell", "dumpsys", service],
                    timeout=20)
            else:
                result = self._execute_command(
                    ["adb", "-s", device_ip, "shell", "service", "list"],
                    timeout=10)
            return {"success": True, "device_ip": device_ip, "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    def cmd_launcher_async(self, device_ip: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "cmd", "shortcut",
    ...
    @async_command
    def emu_sms_send_async(self, device_ip: str, sender: str,
                           text: str) -> dict:
        try:
            self._execute_command(
    ...
    def emu_geo_fix_async(self, device_ip: str, longitude: str,
        ...
        try:
            cmd = ["adb", "-s", device_ip, "emu", "geo", "fix",
                   longitude, latitude]
            if altitude:
                cmd.append(altitude)
    ...
    def getprop_all_async(self, device_ip: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "getprop"], timeout=15)
            return {"success": True, "device_ip": device_ip, "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    @async_command
    def backup_app_async(self, device_ip: str, package: str,
                         save_path: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "backup", "-f", save_path,
                 "-noapk", package], timeout=60)
            return {"success": True, "device_ip": device_ip, "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    @async_command
    def shell_ping_async(self, device_ip: str, host: str,
                         count: str = "4") -> dict:
        try:
            result = self._execute_command(
    ...
    def shell_netstat_async(self, device_ip: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "netstat"], timeout=10)
            return {"success": True, "device_ip": device_ip, "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    @async_command
    def set_inactive_async(self, device_ip: str, package: str,
                           inactive: bool) -> dict:
        try:
            state = "true" if inactive else "false"
            self._execute_command(
                ["adb", "-s", device_ip, "shell", "am", "set-inactive",
                 package, state])
            return {"success": True, "device_ip": device_ip,
                    "package": package, "inactive": state}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    def force_stop_async(self, device_ip: str, package: str) -> dict:
        try:
            result = self._execute_command(
                ["adb", "-s", device_ip, "shell", "am", "force-stop",
                 package])
            return {"success": True, "device_ip": device_ip, "output": result}
        except Exception as e:
            return {"success": False, "device_ip": device_ip, "error": str(e)}
    ...
    KEYCODES = {
        ...
        "MUTE": "91", "PAGE_UP": "92", "PAGE_DOWN": "93",
        "NOTIFICATION": "83", "SETTINGS": "176",
        "APP_SWITCH": "187", "ASSIST": "219",
        "CHANNEL_UP": "166", "CHANNEL_DOWN": "167",
        "TV_INPUT": "178", "TV_POWER": "177",
