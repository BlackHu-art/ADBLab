...
class ADBTesting(ADBModelCore):
    ...
    def take_screenshot_async(self, device_ip: str, save_path: str) -> dict:
        try:
            ...
            self._execute_command(
                ["adb", "-s", device_ip, "shell", "screencap", "-p", temp_path])
            self._execute_command(
                ["adb", "-s", device_ip, "pull", temp_path, save_path])
            self._execute_command(
                ["adb", "-s", device_ip, "shell", "rm", temp_path])
    ...
    def retrieve_device_logs_async(self, device_ip: str, log_path: str) -> dict:
        try:
            log_content = self._execute_command(
                ["adb", "-s", device_ip, "logcat", "-d"])
    ...
    def run_monkey_test_async(
        self, device_ip: str, package_name: str, count: str,
        device_type: str, sanitized_name: str, save_dir: str, index: int,
        callback=None
    ) -> dict:
        def log(msg):
            if callback:
        ...
        try:
            ...
            while monkey_proc.poll() is None:
                try:
                    ...
                    if (current_app != package_name and
                            (time.time() - last_switch_time) > cooldown):
                        ...
                        subprocess.run(
                            ...
                            creationflags=subprocess.CREATE_NO_WINDOW)
                        subprocess.run(
                            ["adb", "-s", device_ip, "shell", "monkey", "-p",
                             package_name, "1"],
                            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                            creationflags=subprocess.CREATE_NO_WINDOW)
    ...
    def kill_monkey_async(self, device_ip: str, index: int) -> dict:
        result = {"device_ip": device_ip, "index": index,
                ...
                if len(parts) > 1:
                    ...
                    try:
                        ...
                        subprocess.check_output(
                            kill_cmd, stderr=subprocess.STDOUT, text=True,
                            creationflags=subprocess.CREATE_NO_WINDOW)
                        result["success"] = True
                        result["message"] = (
                            f"Monkey process (PID: {pid}) successfully killed")
                        return result
                    except subprocess.CalledProcessError as e:
                        result["message"] = (
                            f"Failed to kill monkey process (PID: {pid}): "
                            f"{e.output}")
                        return result
    ...
    @async_command
    def capture_bugreport_async(self, device_ip: str, save_root: str,
                                 index: int, callback=None) -> dict:
        def log(msg):
            if callback:
                callback(f"[{device_ip}] {msg}")
        ...
        target_dir = os.path.join(save_root,
                                  f"{sanitized}_bugreport_{timestamp}")
        os.makedirs(target_dir, exist_ok=True)
        log(f"Created directory: {target_dir}")

        log("Getting Android version...")
        version_cmd = ["adb", "-s", device_ip, "shell", "getprop",
                       "ro.build.version.release"]
        version_proc = subprocess.run(
            version_cmd, text=True, stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            creationflags=subprocess.CREATE_NO_WINDOW)
        ...
        try:
            ...
            return {"device_ip": device_ip, "index": index, "success": False,
        ...
        return {"device_ip": device_ip, "index": index, "success": True,
    ...
    @async_command
    def pull_anr_files_async(self, device_ip: str, sanitized_name: str,
                              save_dir: str, index: int) -> dict:
        try:
            device_anr_dir = os.path.join(save_dir, f"{sanitized_name}_anr")
            os.makedirs(device_anr_dir, exist_ok=True)
            pull_command = ["adb", "-s", device_ip, "pull", "/data/anr",
                           device_anr_dir]
            subprocess.check_output(pull_command, stderr=subprocess.STDOUT,
                                    text=True,
                                    creationflags=subprocess.CREATE_NO_WINDOW)
            return {"device_ip": device_ip, "success": True,
                    "message": f"ANR files saved to {device_anr_dir}",
                    "index": index}
        except subprocess.CalledProcessError as e:
            return {"device_ip": device_ip, "success": False,
